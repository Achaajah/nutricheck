...
export default function FloatingAI() {
  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content:
        "Halo! Saya NutriAI 🌿 Tanya apa saja seputar gizi dan makanan sehat!",
    },
  ]);
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      const reply =
        data.content?.[0]?.text || "Maaf, saya tidak bisa menjawab saat ini.";
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-3 min-h-[280px] max-h-[360px]">
            {messages.map((msg, i) => (
              ...
              >
                <div
                  className={`
                    px-4 py-2 rounded-2xl text-sm max-w-[80%] leading-relaxed
                    ${
                      msg.role === "user"
